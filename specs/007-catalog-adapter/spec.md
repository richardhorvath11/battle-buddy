...
- **Linkage annotations** (`pagerduty.com/service-id`, `github.com/project-slug`): the design's mapping table names them but the six-field model has no matching fields — pinned here as adapter-internal linkage metadata (not consumer-model fields), flagged for design reconciliation. Consumption route: the slices that need paging or repo scoping read these values off the catalog surface, beside the model. *(Corrected during implementation: an earlier draft claimed consumers "receive answers, never the linkage metadata itself", citing the §4 sequence diagram's "deploy window check" arrow. That arrow targets the code capability's MCP participant, not the catalog adapter, so it did not support the claim — and the claim was false anyway, since linkage rides on the returned catalog. The narrow, true claim is the one this bullet opens with: linkage is not a field of the six-field model.)*
