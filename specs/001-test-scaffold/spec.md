...
3. **Given** a repository state where a test layer's directory does not yet contain tests for a future component, **When** verify runs, **Then** it reports the absence visibly (no silent skip) and still evaluates all tests that do exist.
...
**Why this priority**: Co-equal with Story 1 — the contract layer is what makes integration behavior testable hermetically, and later slices (session store, lifecycle, doctor) cannot be built test-first without it. The mock is also the reference against which real service rosters are judged equivalent.
...
1. **Given** the mock in a clean state, **When** a test exercises each required operation of every required capability, **Then** each operation behaves per the contract (records persist and are readable; artifact writes return stable link identifiers; diary entries append and are retrievable most-recent-first).
...
- A test layer directory exists but contains zero tests: verify must surface this visibly rather than reporting green silently.
...
- Very large record payloads (at the contract's documented size boundaries): the mock enforces the same boundary behavior the contract prescribes (e.g. oversized content is rejected or diverted per contract rules), so boundary handling is testable.
...
- **FR-003**: `bb-mock-mcp` MUST implement every required operation of every required capability in the operation contract (record store append/read/update, artifact put returning a stable link, diary append returning a link and recent-entry read, alert context read), with behavior matching the contract's documented semantics. *(Design §7.1)*
...
- **FR-009**: Continuous integration MUST run both layers on every proposed change and block merge on failure. *(AGENTS.md CI commitment)*
- **FR-010**: The mock and all test tooling MUST be dev-only artifacts, excluded from anything shipped to responders. *(Constitution I, Platform Constraints)*
...
- **Mock record store**: In-memory table of session-record-shaped rows; supports append, filtered read, field update; preserves insertion order and per-field history sufficient for conflict tests.
- **Mock artifact store**: In-memory file store keyed by stable generated links; supports put and read-back.
- **Mock diary**: Ordered entry list; supports append (returning a link) and recent-N read.
...
- **Seed fixture**: A declarative description of pre-existing state (records, artifacts, diary entries) loadable into a clean mock.
...
- **SC-006**: A deliberately introduced contract violation in a caller is caught by the contract layer (not by a human reading code) in 100% of the seeded defect cases used to validate the suite.
