...
- **FR-005**: The skill MUST document the drafting handoff with its input contract: the close flow supplies the *computed close-time values* available before any write — in-session evidence links (dashboards, searches, PRs), services/severity, resolution, and the labeled causal proposals (root cause, contributing factors, action items) — plus the locally staged artifact content (pre-upload), including the tool trace and checkpoint history that are the timeline's *sources* (the structured timeline itself derives at §4 step 4, after the diary write, and is a row field, never a draft input); never the written row, which does not yet exist at draft time (drafting precedes the dual-write). Output is entry content in the resolved format; causal-field proposal labels applied by the close flow's labeling rule pass through format matching verbatim — the adapter never strips, rewords, or promotes them. *(§4 close step 1; Constitution V; slice-5 FR-007 — the diary-draft labeling owner)*
