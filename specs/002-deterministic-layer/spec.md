...
- **FR-002**: All five components MUST behave as pure functions of (input payload, local state directory) → (exit code, output) with no network access and no reads beyond the session's local state and paths explicitly named in the input payload (e.g. the runtime's transcript path, FR-012) — hermetically testable by the slice-1 unit layer. *(Design §10; slice-1 FR-007)*
