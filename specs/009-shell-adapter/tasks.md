...
- [ ] T001 Create the shim pair whose existence every later task assumes:
...
- [ ] T002 Extend `tests/unit/test_stdlib_boundary.py`'s allowlists per research R9 and
...
- [ ] T003 [P] Create `tests/helpers/fake_cmux.py` — the fake backend (research R7): a
...
- [ ] T004 Implement the argument grammar and dispatch in `bin/bb_shell.py` per
...
- [ ] T005 Implement config selection in `bin/bb_shell.py` per data-model.md §3 and
...
- [ ] T006 **Create** `tests/unit/test_shell_dispatch.py` — the module T018 and T021 later
...
- [ ] T007 [US1] Implement `DegradedBackend` in `bin/bb_shell.py` — all four methods,
...
- [ ] T008 [US1] Write `tests/unit/test_shell_degraded.py` — SC-002: four verbs × the three
...
- [ ] T009 [US1] Implement `CmuxBackend`'s transport in `bin/bb_shell.py` per research
...
- [ ] T010 [US1] Implement the verb→method mapping in `CmuxBackend` per data-model.md §5:
...
- [ ] T011 [US1] Implement `notify`'s level encoding per research R4: `notification.create`
...
- [ ] T012 [US1] Write `tests/unit/test_shell_cmux.py` — the request envelope
...
- [ ] T013 [US2] Implement the fail-soft wrapper in `bin/bb_shell.py` (FR-005): a single
...
- [ ] T014 [US2] Write `tests/unit/test_shell_failsoft.py` — the **24-case matrix**
...
- [ ] T015 [US2] Add the per-invocation independence test to
...
- [ ] T016 [US2] Add the no-hang assertion to `tests/unit/test_shell_failsoft.py`: the
...
- [ ] T017 [US3] Add the captured-traffic scan to `tests/unit/test_shell_failsoft.py` per
...
- [ ] T018 [US3] Add the structural grammar assertion to `tests/unit/test_shell_dispatch.py`
...
- [ ] T019 [P] [US4] Write `bin/bb-shell.md` — the backend-independent adapter contract
...
- [ ] T020 [P] [US4] Write `bin/bb-shell.cmux.md` — the cmux backend mapping: socket
...
- [ ] T021 [US4] Add the documentation gates to `tests/unit/test_shell_dispatch.py`:
...
- [ ] T022 Amend `bb-technical-design.md` per spec FR-001 and Assumptions ("flagged for
...
- [ ] T023 Update `AGENTS.md`'s build-order line to mark slice 9 landed. **Conflict note**:
...
- [ ] T024 Run quickstart.md's manual validation against a live cmux (macOS): all four
