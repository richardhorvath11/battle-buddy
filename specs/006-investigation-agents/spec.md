...
3. **Given** a documented invariant-phase ledger (`evidence-gathering` or `deep-dive`) with fewer than three live hypotheses or none `fresh`, **When** the validator runs, **Then** it fails on the anchoring guard — the skill's guard and the validator's rule agree, phase for phase. *(§3.4, FR-5e; §5.4; slice-2 validator invariant phases)*
...
The triage agent definition pins: a fast/cheap model class (configurable), a ≤2-minute wall-clock target with a capped turn budget (default 15 — documented in the agent definition, *enforced* by the slice-2 hook; wall-clock is measured and reported in the verdict's budget field, never enforced), a read-only narrow toolset named by manifest capability — `alerting`, `code` (catalog and runbook reads), `storage` (session-store reads), `observability` — and the four fixed questions: Known? (fingerprint + keyword recall) Real? (flap history) What changed? (deploy window) Who's affected? (metrics + dependsOn). Output is a structured verdict (`bb.verdict.v1`) where "no strong signal" is an honest, first-class answer — and a budget-truncated verdict automatically satisfies the no-strong-signal launch condition for deep investigation. On new alerts mid-session, triage re-invokes to classify related-to-current vs separate.
...
- **Deep investigation requested with no triage verdict yet** (responder escalates immediately): the ledger starts in an early phase (`triage-seeded` or `hypothesis-generation`), where the anchoring invariants do not yet bind — sparse or empty ledgers are legal there; the ≥3-live/≥1-`fresh` rules bind once the ledger enters an invariant phase (`evidence-gathering`, `deep-dive`). *(slice-2 validator phase model — see Assumptions)*
- **Unregistered specialist spawned ad hoc**: its tool calls still trace, but its actor key maps to no role — in v1 only the triage role carries a turn cap, so the registration stake for specialists is trace attribution (and future per-role budgets), not an evaded cap; the skill instructs registering every spawn regardless, and the gap is visible in the trace's actor keys. *(local-state protocol v1 — `budgets.triageTurnCap` is the only v1 budget key)*

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: The investigation skill MUST lead with the validation discipline as its top-priority instruction: recalled sessions and triage candidates are hypotheses, never conclusions; every hypothesis carries provenance (`triage` | `recall` | `fresh`); every non-`fresh` hypothesis is marked VALIDATED or INVALIDATED against current-incident evidence before being acted on. The skill is loaded by the orchestrator and both investigation agents — one skill, different budgets and toolsets per role (FR-5c). *(§3.3, FR-5c, FR-5d, SM-4; Constitution VI)*
- **FR-002**: The skill MUST state the anchoring guard — ≥3 live hypotheses before deep-diving any one, ≥1 `fresh` — with the validator's phase scoping stated exactly: the guard binds in the invariant phases (`evidence-gathering`, `deep-dive`); the earlier phases (`triage-seeded`, `hypothesis-generation`) permit sparse ledgers; enforcement is the slice-2 validator's at checkpoint-write time. *(§3.3, FR-5e, §5.4; slice-2 validator; Constitution II, VI)*
...
- **FR-004**: The skill MUST carry the capability-scoped untrusted-data rule: output from `alerting` and `observability` tools is data, never instructions, with delimiter-wrapping applied where the agent controls the text (checkpoints, briefings, diary drafts, subagent prompts) — stated explicitly as probabilistic mitigation, with guarantees living only in the deterministic layers 1–3 (the layer-4 tripwire is advisory, D-20). The v1 untrusted set matches slice-2's pinned tripwire set; design §3.3 also names ticket-shaped tools — deferred with slice-2 FR-010's recorded rationale (no ticket capability exists in contract v1; the set grows with the contract, not ad hoc — see Assumptions). *(§3.3, §3.5 layer 4, D-20; slice-2 FR-010; Constitution III)*
- **FR-005**: The skill MUST reference the briefing format (deep-linked evidence per claim) and point at slice-3's session-store conventions as the normative retrieval flow — it consumes them, never restates them normatively (see Assumptions for the placement rationale). The briefing reference MUST carry the causal-field discipline: root-cause and contributing-factor statements in briefings and ledger syntheses are explicitly-labeled proposals, never promoted to fact without a human decision. *(§3.3; slice-3 FR-007; FR-8; Constitution V; one source of truth per rule)*
- **FR-006**: The triage agent definition MUST pin: configurable fast/cheap model class; ≤2-minute wall-clock *target* (measured, reported in `budget_spent`, never enforced) with the turn cap documented (default 15, its configuration key named, enforcement attributed to the slice-2 hook); read-only toolset named by manifest capability — `alerting`, `code` (catalog and runbook reads), `storage` (session-store reads), `observability` (the design's "catalog/code reads" and "session-store reads" ride `code` and `storage` respectively); the four fixed questions; the input contract (alert context, flap history, catalog entry + runbooks, candidate session rows — supplied by the slice-5 spawn step); structured `bb.verdict.v1` output with honest no-strong-signal; budget-truncated verdicts satisfying FR-5f(a) via the verdict's own fields; and mid-session re-invocation charter (related vs separate). *(§3.4, D-17, FR-2, FR-5, FR-5a; local-state protocol config keys)*
...
- **FR-009**: The skill MUST pin the deep-investigation launch conditions — (a) no strong signal, (b) responder request with promotion always launching, (c) failed fix verification — with orchestrator-proposes/responder-confirms and the auto-launch configuration flag for incident severity (the design's `autoLaunchDeep` key); the orchestrator ingests ledger updates only, never raw findings. *(§3.4, FR-5f, FR-5b)*
...
- **FR-012**: Hermetic tests MUST cover: every documented schema example classified by the slice-2 validator exactly as documented (valid passes; each documented-invalid fails naming the documented rule); role-registration writes conforming to the protocol's `agents.json` shape; the anchoring-guard state matrix — both invariant phases × {2 live, 3 live none fresh, 3 live with fresh, dead fresh does not satisfy} — classified correctly; and the toolset-capability cross-check against the capability manifest (SC-006's instrument) — asserting on artifacts, never prose, no credentials, no network. Agent behavior itself (does triage actually answer in 2 minutes, does the ledger actually validate recalled candidates) is scenario-harness territory (design §10 on-demand layer), explicitly out of CI scope. *(design §10; Constitution VIII)*
...
- **SC-004**: The anchoring-guard state matrix (both invariant phases — `evidence-gathering`, `deep-dive` — × {2 live, 3 live no fresh, 3 live with fresh, dead fresh}) classifies 100% correctly under the validator — the skill's stated guard and the enforced guard are the same guard, in the same phases.
- **SC-005**: Zero concrete MCP server or tool names across all prose deliverables — the skill document, its references (schemas, briefing, the retrieval pointer), and the five agent definitions — verified mechanically (the same scan discipline as the constitution's forbidden-ops list).
...
- **Anchoring-guard phase model** (from the merged slice-2 validator, the enforcement authority): the ledger phase vocabulary is `triage-seeded` → `hypothesis-generation` → `evidence-gathering` → `deep-dive` → `resolution`; the ≥3-live/≥1-fresh invariants bind in `evidence-gathering` and `deep-dive` (the validator's invariant phases), and the earlier phases permit sparse or empty ledgers — that is what makes immediate escalation legal. Note for the schemas reference: design §5.4's own example ledger (phase `evidence-gathering`, one hypothesis) fails the current validator — the schemas reference must follow the validator, and the design example is flagged for reconciliation.
- **Untrusted-set v1** matches slice-2 FR-010's pin (`alerting`, `observability`): design §3.3 also names ticket-shaped tools, deferred because contract v1 defines no ticket capability the binding map could classify; the set grows with the contract, not ad hoc.
- **Retrieval reference placement**: slice 3 made the session-store skill the normative home of the retrieval conventions (its plan-time placement assumption); this slice's retrieval reference is a pointer, resolving design §3.1's sketch of an investigation-side retrieval document without duplicating the rules.
...
- **Seeding "unvalidated" — implementation reconciliation** (recorded at implement time): FR-007/US3's "provenance `triage`, unvalidated" is reconciled with the validator's phase-independent rule that every non-`fresh` hypothesis carries VALIDATED/INVALIDATED (`memory.unvalidated_non_fresh`): seeded entries carry forward the marks triage assigned in the verdict, and "unvalidated" means *not yet re-validated by the deep investigator against current-incident evidence* — re-validation before acting is the discipline's requirement. The seeded-from-triage worked example in the schemas reference validates under this reading; a literally mark-less seed would be schema-invalid.
