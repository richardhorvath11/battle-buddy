...
7. **Given** close completes, **When** the last step runs, **Then** the shell workspace is closed through the adapter with state restorable — a printed message in degraded mode. *(§4 close diagram step 33, §6.3, FR-3)*
...
- **Marker exists but its open write was never confirmed** (crash between marker write and row append): the next open command detects the unconfirmed marker at its existing-marker step, surfaces it, and overwrites it only on responder confirmation — a marker *rewrite*, not a clearance; deletion-is-cleared remains exclusive to confirmed close. The session guard's warning covers the crashed session's own end. *(informed default; D-11; local-state protocol v1)*

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: `/page <alert-id>` MUST execute the documented open flow in order: cheap preflight (config presence; green-stamp match per slice-4 semantics — no probes on the happy path; missing config stops with "run /setup"; missing/stale stamp auto-runs responder-mode setup), existing-marker handling (a marker with a confirmed open write means a session is already open — surface it and stop, offering close-or-continue; an unconfirmed marker is crash residue, surfaced and overwritten only on responder confirmation), session-ID computation (`{type}-{source-id}-{ISO date}`), marker write, shell workspace open, alert context + flap history fetch, catalog resolve + fresh runbook and dashboard fetch, tier-0 retrieval, triage spawn, verdict validation, checkpoint-zero persist (riding the open-time append as the row's verdict field — not a separate store write; an overflowed verdict stores its artifact first), row append (`status: open`), row read-back, marker confirmation, briefing. *(§3.2; D-8; NFR-1)*
- **FR-002**: The commands are the local-state protocol's designated marker writer: marker created at open with the protocol's fields; `open_write_confirmed: true` set only after the open-time append's read-back; the whole local session directory deleted only by a confirmed close — this slice writes exactly the protocol shapes slice 2 reads. On the join path the marker is rewritten to the joined session's ID, and its confirmation is the take-over write's read-back — a documented extension of the protocol's append-read-back rule (see Assumptions). *(local-state protocol v1; D-11; Constitution II)*
...
- **FR-008**: After approval, `/close` MUST execute the slice-3 dual-write in order — diary (capturing its link) → artifact upload → row update with all close-time fields → row read-back — deleting the local session directory only on read-back success; on diary failure the row still lands with `diary_pending: true` and a queued retry; a transiently failed row write is retried and the close flow blocks on row-write success (§9's rate-limit/write-failure row) — except under ownership displacement, where close goes read-only per FR-010 instead; on both paths, deletion never precedes a successful read-back. Close's final step closes the shell workspace through the adapter, state restorable. *(§4 close steps 2–6 and diagram step 33; §9; FR-4b; slice-3 FR-008; local-state protocol v1)*
- **FR-009**: Close-time artifacts MUST be the hook-captured files uploaded mechanically — the session transcript captured from the runtime's transcript path at close (protocol staging populates only at session end — see Assumptions), the local trace under its documented uploaded name, the checkpoint history, and the generated report — with the structured timeline derived from the tool trace plus checkpoint history, never from prose recall. *(§4 close steps 3–4; D-5, D-12; slice-3 FR-004; local-state protocol v1 staging)*
- **FR-010**: `/close` MUST merge duplicate open rows sharing the source ID: earliest row canonical, artifact links folded in, duplicate marked `superseded`; and close's row writes MUST honor the slice-3 pre-write ownership check, going read-only with a take-over report when displaced — an extension of slice-3's checkpoint-write-scoped rule to the close-time update (see Assumptions). *(§4, D-18; slice-3 FR-009)*
...
- **SC-002**: In 100% of simulated happy-path opens, the preflight re-runs neither setup nor doctor — asserted on artifacts: the stamp file is byte-unchanged and no doctor-report artifact is produced for the run — and the marker reaches `open_write_confirmed: true` only after the row read-back.
...
- **SC-006**: In 100% of close drafts, causal fields carry the explicit proposal label, and the draft's causal values (root cause, contributing factors, action items) appear only under proposal-labeled fields — a structural property asserted on the draft artifact.
...
- **Crashed-open recovery** (marker present, open write never confirmed, session gone): the next open command detects the unconfirmed marker at its existing-marker step — before any store read — surfaces it, and rewrites it only on responder confirmation (FR-001; the crashed-open edge case). Informed default; D-11's guard covers the crashed session's own end, and the protocol's one-session rule needs an unbrick path.
- **Preflight stamp semantics are slice 4's** (spec authored on PR #9, unmerged at this writing — dependency declared): slice 4 pins staleness as matching-only (plugin version + roster hash) and flags the design's §3.2 "fresh and matching" wording as resolved matching-only there; this slice consumes that pin, never redefines it.
- **Transcript availability at close**: the local-state protocol stages the transcript only at session end, yet close — which runs mid-session — uploads it and then deletes the staging directory. This spec resolves the upstream timing conflict by capturing the transcript from the runtime's transcript path directly at close; turns after close are absent from the uploaded transcript (accepted v1 limitation). Flagged for local-state-protocol reconciliation (a protocol change requires its version bump).
- **Join-path marker semantics are a protocol extension**: the protocol defines `open_write_confirmed` for the open-time *append* read-back only; join performs an ownership *update*. Pinned: marker rewritten to the joined session's ID, confirmed by the take-over write's read-back. Flagged for protocol reconciliation.
- **Ownership check at close is an extension**: design §4/§9/D-18 and slice-3 FR-009 scope the pre-write re-read to *checkpoint* writes; extending it to the close-time row update is this spec's pin, following D-18's intent (no write to a row you no longer own).
- **Degraded-mode workspace open**: design §3.2 step 3 says "no-op in degraded mode" while §6.3 says every degraded call is "a printed link or message"; the spec follows §6.3 (a printed message) and flags the design-internal inconsistency here.
- **Alerting-failure stand-in**: the mock's alerting capability is seed-only with a closed error set; the hermetic stand-in for alert-fetch failure is a `not_found` on an unseeded alert. Richer fault injection is plan-time mock work if the distinction (broken tool vs unknown alert) proves test-relevant.
