...
A team runs `/doctor` outside any incident, or after changing its MCP roster. For each required operation in the capability manifest, doctor inspects the connected MCP tools' schemas, matches the operation to a concrete tool semantically, confirms the match with a benign, read-shaped probe (mutating operations: schema match now, end-to-end exercise in setup's smoke test), and writes the resolved binding map — entries in the exact `capability.operation` → tool-name format the deterministic layer reads — into the workspace configuration, committed with the workspace repo. Required capability unsatisfied → fail loudly naming the gap; optional capability missing → a "reduced features" report listing exactly which features are disabled. Doctor also verifies config validity (store reachable with the expected header row, diary readable with its append operation schema-matched, catalog repo parseable), shell-notification round-trip when a shell adapter is configured, and version-seam integrity — config-block and store-schema versions compatible with the installed plugin, reporting the exact migration otherwise.
...
**Independent Test**: From a fixture state with valid team scope and absent responder scope, run responder-mode steps against the mock: assert the stamp lands with its three fields, no mutating operation touches team resources (write log unchanged), and the run's structured report (FR-004) records a probe outcome per required operation; then assert a plugin-version or roster-hash change invalidates the stamp.
...
- **Doctor run mid-incident**: the design says run outside incidents; doctor's writes are bindings, config, and stamp — never session rows — so an accidental mid-incident run degrades to noise, not damage. *(§7.2; informed default — the design pins only "run outside incidents")*
...
- **FR-001**: The plugin MUST ship a capability manifest declaring required capabilities (`storage`, `artifacts`, `diary`, `alerting`) and optional capabilities (`code`, `observability`) with their operations and input/output shapes ("names don't matter; shapes do") and, for optional ones, the features they enable. The required half is the shipped projection of operation contract v1 (authored in slice-1 operations.md, encoded as `tools/bb-mock-mcp/contract.json`); the optional half comes from design §7.1, since contract v1 defines no optional operations yet (see Assumptions). *(§7.1; slice-1 operations.md "Slice 4's shipped manifest/capabilities.json derives from this artifact")*
- **FR-002**: `/doctor` MUST resolve each required operation to a concrete tool by inspecting connected MCP tools' schemas, MUST confirm each match with a benign, read-shaped probe — mutating operations are confirmed by schema match at doctor time and exercised end-to-end by setup's smoke test (see Assumptions) — MUST surface any multi-match ambiguity (more than one tool matching an operation) for an explicit choice, never a silent pick, and MUST write the resolved binding map into the workspace configuration in the exact `capability.operation` → tool-name entry format the slice-2 local-state protocol's config table defines — the map is committed with the workspace repo. *(§7.2, D-13; local-state protocol v1)*
- **FR-003**: `/doctor` MUST verify, per run: probes pass under the current responder's credentials; the shell-notification round-trip when a shell adapter is configured (skipped-not-failed otherwise); config validity — store reachable with the expected header (the slice-3 schema documentation is the authority), diary readable with its append operation schema-matched (writability proper is exercised by setup's smoke test — see Assumptions), catalog repo parseable; and version-seam integrity — config-block and store-schema versions compatible with the installed plugin, reporting the exact migration needed otherwise. *(§7.2, §2.1, D-16)*
- **FR-004**: Doctor report semantics MUST be: any required capability unsatisfied → loud failure naming the specific gap; optional capability missing → green run with a reduced-features list naming exactly what is disabled; dependent features degrade at runtime, never error. The report is itself a structured artifact — one machine-readable outcome per resolved operation and per verification check (shape pinned at plan time) — so tests assert on it, never on prose. *(§7.2; report-as-artifact is a spec pin serving Constitution VIII)*
- **FR-005**: A green doctor run MUST write the local last-green-doctor stamp — timestamp, plugin version, roster hash — never committed to the repo; the stamp is stale whenever the plugin version or roster hash differs from current state — the sole staleness rules in v1; the timestamp is informational, reported but never expiry-checked (a time window would reintroduce 3am probes — see Assumptions). The `/page` preflight that trusts it ships in slice 5; this slice defines the stamp's content and staleness rules. *(§7.2, §3.2 step 1, D-15, §2.1)*
...
- **FR-007**: Team mode MUST: resolve bindings; create the session store from the slice-3 documented schema, writing the header row through the just-resolved storage binding, or validate an existing store's header reporting exact mismatches; establish the artifact root (folder creation reduces to the name-path convention over the artifact write operation — see Assumptions); prompt for diary location and catalog repo; write the full config block including its version field; scaffold the workspace repo with its §2.1-mandated contents — the config block with plugin pin, the team MCP roster file with tokens as environment-variable references (secrets never enter the repo), the resolved binding map, and a README — zero upstream content, with push-to-private-org instructions; and finish with `/doctor` plus an end-to-end smoke test — a `session_type: test` session exercising record append, artifact write, diary append, and read-back. *(§7.3, §2.1, D-16)*
...
- **FR-009**: Running `/setup` on an already-green workspace MUST validate and report — zero mutating operations — and on partial state MUST do only what is missing; running twice is always safe. A malformed config block MUST be surfaced as an explicit repair case, never treated as absent. *(§7.3)*
...
- **FR-011**: Every requirement above MUST be exercised by hermetic contract tests against `bb-mock-mcp`: binding resolution against the mock's schema-registry surface (`describe()`, built by slice 1 for exactly this), setup create-vs-validate paths, smoke-test exclusion from retrieval, doctor failure modes (required-missing loud, optional-missing reduced), and stamp lifecycle — asserting on artifacts (rows, write log, stamp fields, the structured doctor report — the oracle for read-shaped probe outcomes the write log cannot see), never prose, with no credentials and no network. Where contract v1 offers no surface (header rows, folder roots, catalog state), tests exercise the decision protocol against fixture surfaces pinned at plan time (see Assumptions). *(design §10 layer 2; slice-1 FR-011; Constitution VIII)*
...
- **Doctor report**: The structured per-check outcome artifact each doctor run produces (FR-004); the test oracle for read-shaped probe outcomes, which no write log can see.
...
- **SC-003**: Create-vs-validate: from an empty fixture store the created header representation matches the documented column list exactly; from a correct existing store, validation passes with zero writes; from a mismatched store, the exact mismatch is reported with zero writes — each in 100% of simulated runs.
...
- **Benign probes are read-shaped**: doctor probes use read operations — storage via a filtered record read, diary via reading recent entries, alerting via a benign history-filter query where an empty result passes (reachability and shape, not data presence). Mutating operations are verified by schema match at doctor time and exercised end-to-end only by setup's smoke test; consequently doctor verifies the diary *readable with append schema-matched* rather than §7.2's literal "diary writable". Spec-pinned defaults; the design says "benign probe call" without defining benignity for mutating operations.
- **Manifest optional half**: contract v1 declares only the four required capabilities; the `code`/`observability` operations and their `enables` lists come from design §7.1. Extending the operation contract with optional-op shapes is plan-time work for this slice (or an explicit deferral recorded there).
- **Surfaces contract v1 lacks**: the mock's record store has no header concept, and artifacts have no folder operation — folders are name-path conventions over the artifact write operation (slice-3's `battle-buddy/<session_id>/` prefix), so "establish the artifact root" reduces to validating writability and recording the root location in config. Header create-vs-validate tests exercise the decision protocol (create on empty, zero-write validation, exact-mismatch reporting) against a fixture store surface whose header representation is pinned at plan time. Catalog reachability/parseability checks use the scenario fixture catalog repo (design §10).
- **Reporting granularity is spec-strengthened**: "reporting exact mismatches" (header validation) and "naming the stale entries" (drift) elaborate the design's bare "validate an existing Sheet's header" and "flags drift".
...
- **Roster hash input** (which roster facts are hashed) and the **stamp's on-disk location** are pinned at plan time; the stamp lives with the gitignored runtime droppings per §2.1. Design §3.2's "fresh and matching" is pinned as matching-only in v1 — no time-based expiry; the timestamp field is diagnostic.
