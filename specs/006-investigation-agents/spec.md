...
- **FR-006**: The triage agent definition MUST pin: configurable fast/cheap model class; ≤2-minute wall-clock *target* (measured, reported in `budget_spent`, never enforced) with the turn cap documented (default 15, its configuration key named, enforcement attributed to the slice-2 hook); read-only toolset named by manifest capability — `alerting`, `code` (catalog and runbook reads), `storage` (session-store reads), `observability` (the design's "catalog reads" and "session-store reads" ride `code` and `storage` respectively); the four fixed questions; the input contract (alert context, flap history, catalog entry + runbooks, candidate session rows — supplied by the slice-5 spawn step); structured `bb.verdict.v1` output with honest no-strong-signal; budget-truncated verdicts satisfying FR-5f(a) via the verdict's own fields; and mid-session re-invocation charter (related vs separate). *(§3.4, D-17, FR-2, FR-5, FR-5a; local-state protocol config keys)*
...
- **SC-005**: Zero concrete MCP server or tool names across all prose deliverables — the skill document, its references (schemas, briefing), and the five agent definitions — verified mechanically (the same scan discipline as the constitution's forbidden-ops list).
