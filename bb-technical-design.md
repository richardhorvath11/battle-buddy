...
**Shipped prose abstracts the MVP binding above** (slice 8). The heading names a concrete document product and its MCP; Constitution VII supersedes that framing for anything shipped, so `skills/diary/` says "the team's configured diary, through the diary capability" and names no product — the binding map makes the store a binding-time fact, and the concrete name is recorded here (where it is architectural record, not shipped instruction) and in `templates/mcp.recommended.json`. The one exception the shipped prose does carry is the *deferral* list — naming Confluence, Notion and git-markdown as adapters this version does **not** integrate with, which FR-4a requires and which is the opposite shape from the integration-by-name Constitution VII forbids; the capability-naming scan masks exactly that sentence.

**Skill layout**: `skills/diary/SKILL.md` plus `references/format.md` — extending §3.1's bare `diary/SKILL.md` on the `session-store/` and `investigation/` sibling precedent shown in that same tree, the same expansion slice 7 made for `catalog/`. **Config keys**: `battleBuddy.diary.template` (pinned by slice 5) and `battleBuddy.diary.recentEntries` (slice 8; default 5).
...
| D-23 | **Diary format resolution pinned** (slice 8): read depth **5** behind `battleBuddy.diary.recentEntries`; a template is *malformed* only if its configured value is a non-string or blank (absent ≠ malformed), and a malformed one falls back to match-recent **with the problem surfaced**, never blocking the draft; `content` is the sole extraction surface and `at` is **never** read as the team's date format; the **entry title is excluded from the compared structure**; date-token padding is a property of how a component is *written*, not of its value; a line already written in the pattern language is date-bearing and its pattern is itself; numeric-date ambiguity resolves **pass-level** across the entries read and is otherwise **surfaced, never silently picked**; consistency detection across the read is what makes freshest-wins observable; and format matching is **opaque-block arrangement**, never authoring | Every one of these is a rule §6.2 states loosely or not at all, and each was undecidable enough that two implementers would diverge. The title exclusion is the sharpest: a diary title carries that entry's own date and incident name, so including it makes *every* real diary compare as inconsistent — the rule looks like a detail and is actually what makes the whole comparison work. Ambiguity surfacing extends D-22's posture ("a silent pick is the failure mode a responder cannot detect") to a second surface. Opaque-block arrangement is what turns Constitution V's causal-label pass-through from a promise into a property that holds for every block, not just the ones a test names. **Recorded honestly alongside**: (a) two places where slice 8's spec and `commands/close.md` describe the drafting handoff differently, neither resolved by this slice: §4 step 4 places the structured-timeline derivation *after* the diary write while `commands/close.md` produces it *before* the dual-write, and the two draft-input lists diverge in **both** directions — close.md fills responder and the open/close timestamps that FR-005 omits, while FR-005 names a resolution value and the locally staged pre-upload artifact content that close.md's draft anatomy does not yet enumerate. Slice 8 asserts neither ordering and defers to close.md on the input enumeration — its prose names the categories it receives and explicitly does not compete with close.md's list, which stays normative for what the close flow fills; (b) three pins have no landed consumer yet — `battleBuddy.diary.recentEntries` (slice 5's close flow hardcodes the depth), the four-kind notice vocabulary (nothing in `commands/close.md` receives notices), and slice 5's `_render_draft_entry` stand-in |
