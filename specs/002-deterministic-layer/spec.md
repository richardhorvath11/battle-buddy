...
2. **Given** a benign tool call (including near-miss strings inside quoted text or file paths), **When** the hook evaluates it, **Then** it is allowed with no side effects.
...
2. **Given** the triage agent at its configured turn cap, **When** it attempts another tool call, **Then** the call is denied with the emit-your-verdict message, and a truncated-verdict marker is recorded so downstream logic can apply the no-strong-signal rule (design FR-5f(a)).
...
2. **Given** a cleared marker (or no marker — no session was opened), **When** the session ends, **Then** the guard stays silent and the session ends normally.
3. **Given** a transcript path in the hook input, **When** the session ends, **Then** the transcript file is copied to the session's staging area; a missing/unreadable transcript degrades to a logged notice, never a session-ending failure.
...
- Deny patterns appearing inside legitimate content (a quoted command in a commit message, a URL containing "rm -rf"): over-match is acceptable and by design (Constitution III) — but the fixture corpus must include benign near-misses so over-matching stays deliberate, not sloppy.
...
- **FR-002**: All five components MUST behave as pure functions of (input payload, local state directory) → (exit code, output) with no network access and no reads outside the session's local state — hermetically testable by the slice-1 unit layer. *(Design §10; slice-1 FR-007)*
...
- **FR-006**: `bb-validate` MUST validate documents against their embedded version tag (`bb.verdict.v1`, `bb.ledger.v1`): schema shape plus the semantic invariants — non-`fresh` hypotheses carry validation status; ≥3 live hypotheses and ≥1 `fresh` before any deep-dive marker; evidence entries are `{url, excerpt}` pairs — reporting all violations in one pass as a machine-readable list, exiting non-zero on any violation, and never modifying input. *(Design §5.4, D-14; Constitution VI)*
...
- **FR-008**: The trace hook MUST append one structured line per tool call (all agents) to the session's local trace file: sequence number, agent, tool, capability where known, timestamp, payload summary — append-only, no gaps or duplicates. *(D-12; design §5.3)*
- **FR-009**: The trace hook MUST enforce the configured triage turn cap by denying further tool calls past the cap with the budget-exhausted/emit-your-verdict message, recording a truncated-verdict marker for downstream FR-5f(a) handling; absent configuration, a documented default cap applies. *(D-17; design §3.4)*
- **FR-010**: The trace hook MUST implement the injection tripwire: results from untrusted-capability tools matching the documented instruction-shaped heuristics trigger one advisory data-not-instructions reminder and one trace log entry; the mechanism is advisory only and documented as probabilistic. *(D-20; design §3.5 layer 4)*
- **FR-011**: The session guard MUST detect an opened-but-unconfirmed session record via the local marker protocol at session end and warn/block loudly with the remedial instruction; it MUST stay silent when no session was opened or the record was confirmed. *(D-11; design §4)*
...
- **FR-013**: The marker and trace file formats, locations, and lifecycle MUST be documented as a versioned local-state protocol — slice 3's conventions and slice 5's commands build on them without reverse-engineering the code. *(Constitution IV-adjacent: contracts over prose)*
...
- **Session marker**: Local file recording session identity and record-write confirmation state; created at open, updated on confirmed write, cleared at confirmed close.
- **Trace line**: One structured record per tool call: `{seq, agent, tool, capability?, at, summary}`.
...
- **SC-003**: The fingerprint golden corpus passes 100% on both the oldest (3.9) and newest supported Python versions, and a seeded normalization change without a version bump fails it.
...
- The deny hook does not implement an inline named-reason bypass in v1 — the design specifies outright blocking for deny classes (FR-7a); a push-gate-style bypass pattern (family-meals lineage) may arrive with a later dev-workflow slice, distinct from these runtime guardrails.
- Auth-error context for the credential-scanning deny class comes from the session's own trace file (recent entries), not from external state — keeping the hook hermetic.
